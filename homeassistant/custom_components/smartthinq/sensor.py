...
def setup_platform(hass, config, add_entities, discovery_info=None):
    ...
    for device_id in hass.data[KEY_SMARTTHINQ_DEVICES]:
        ...
        if device.type == wideq.DeviceType.DISHWASHER:
            ...
            try:
                devices.append(LGDishWasherDevice(client, device, base_name))
        ...
        if device.type == wideq.DeviceType.WASHER:
            base_name = "lg_washer_" + device.id
            LOGGER.debug("Creating new LG Washer: %s" % base_name)
            try:
                devices.append(LGWashingMachineDevice(client, device, base_name))
            except wideq.NotConnectedError:
                # Dishwashers are only connected when in use. Ignore
                # NotConnectedError on platform setup.
                pass
...
class LGWashingMachineDevice(LGDevice):
    def __init__(self, client, device, name):
        """Initialize an LG washing machine Device."""

        super().__init__(client, device)

        # This constructor is called during platform creation. It must not
        # involve any API calls that actually need the washing machine to be
        # connected, otherwise the device construction will fail and the entity
        # will not get created. Specifically, calls that depend on washing machine
        # interaction should only happen in update(...), including the start of
        # the monitor task.
        self._washingMachine = washer.WasherDevice(client, device)
        self._name = name
        self._status = None
        self._failed_request_count = 0
    ...
    @property
    def state_attributes(self):
        """Return the optional state attributes for the washing machine."""
        data = {}
        data[ATTR_DW_REMAINING_TIME] = self.remaining_time
        data[ATTR_DW_REMAINING_TIME_IN_MINUTES] = self.remaining_time_in_minutes
        data[ATTR_DW_INITIAL_TIME] = self.initial_time
    ...
    def state(self):
        if self._status:
            if (self._status.state == washer.WasherState.ADD_DRAIN):
                return 'drain'
            elif (self._status.state == washer.WasherState.COMPLETE):
                return 'complete'
            elif (self._status.state == washer.WasherState.DETECTING):
                return 'detecting'
            elif (self._status.state == washer.WasherState.DETERGENT_AMOUNT):
                return 'detergent amount'
            elif (self._status.state == washer.WasherState.DRYING):
                return 'drying'
            elif (self._status.state == washer.WasherState.END):
                return 'end'
            elif (self._status.state == washer.WasherState.ERROR_AUTO_OFF):
                return 'error auto off'
            elif (self._status.state == washer.WasherState.FRESH_CARE):
                return 'fresh care'
            elif (self._status.state == washer.WasherState.FROZEN_PREVENT_INITIAL):
                return 'frozen'
            elif (self._status.state == washer.WasherState.FROZEN_PREVENT_PAUSE):
                return 'frozen'
            elif (self._status.state == washer.WasherState.FROZEN_PREVENT_RUNNING):
                return 'frozen'
            elif (self._status.state == washer.WasherState.INITIAL):
                return 'initial'
            elif (self._status.state == washer.WasherState.OFF):
                return 'off'
            elif (self._status.state == washer.WasherState.PAUSE):
                return 'paused'
            elif (self._status.state == washer.WasherState.PRE_WASH):
                return 'pre wash'
            elif (self._status.state == washer.WasherState.RESERVE):
                return 'reserve'
            elif (self._status.state == washer.WasherState.RINSING):
                return 'rinsing'
            elif (self._status.state == washer.WasherState.RINSE_HOLD):
                return 'rinsing (hold)'
            elif (self._status.state == washer.WasherState.RUNNING):
                return 'running'
            elif (self._status.state == washer.WasherState.SMART_DIAGNOSIS):
                return 'smart diagnosising'
            elif (self._status.state == washer.WasherState.SMART_DIAGNOSIS_DATA):
                return 'smart diagnosising'
            elif (self._status.state == washer.WasherState.SPINNING):
                return 'spinning'
            elif (self._status.state == washer.WasherState.TCL_ALARM_NORMALTUBCLEAN_COUNT_ALARM):
                return 'alarm'
            else:
                return 'unknown'
        return 'off'
    ...
    def remaining_time_in_minutes(self):
        ...
        # reality.
        if (self._status and
            (self._status.state == washer.WasherState.OFF or
             self._status.state == washer.WasherState.COMPLETE)):
            return 0
        return self._status.remaining_time if self._status else 0
    ...
    @property
    def initial_time_in_minutes(self):
        # When in state OFF, the washing machine still returns the initial program
        # length of the previously ran cycle. Instead, return 0 which is more
        # reflective of the dishwasher being off.
        if (self._status and
            self._status.state == washer.WasherState.OFF):
            return 0
        return self._status.initial_time if self._status else 0
    ...
    def _restart_monitor(self):
        try:
            self._washingMachine.monitor_start()
        except wideq.NotConnectedError:
            self._status = None
        except wideq.NotLoggedInError:
            LOGGER.info('Session expired. Refreshing.')
    ...
    def update(self):
        """Poll for washing machine state updates."""
        ...
        # On initial construction, the washing machine monitor task
        # will not have been created. If so, start monitoring here.
        if getattr(self._washingMachine, 'mon', None) is None:
            self._restart_monitor()

        try:
            status = self._washingMachine.poll()
        except wideq.NotConnectedError:
            self._status = None
            return
        except wideq.NotLoggedInError:
            ...
            LOGGER.info('Key error. Refreshing.')
            self._client.refresh()
            self._restart_monitor()
            return
